...
const Notifications=()=>{
    ...
    const elder_notification= async data=>{ 
       
        try{      
            const response= await axios({
                method: 'post',
                url: baseUrl+'api/v0.1/elder-notifications',
                headers:{'Content-Type': 'application/json','Authorization': 'Bearer ' + user_token},
                data,
                }).then(function (response) {
                  if (response.data.status === "success") {
                    response.data
                    setEldernotifications(response.data.data)
                    setIsLoading(false);
                  }
                },200) 
        } 
